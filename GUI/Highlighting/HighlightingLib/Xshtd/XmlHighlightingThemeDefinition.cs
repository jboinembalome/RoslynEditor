namespace HighlightingLib.Xshtd
{
	using HighlightingLib.HighlightingTheme;
	using HighlightingLib.Xshtd.interfaces;
	...
	internal class XmlHighlightingThemeDefinition : IHighlightingThemeDefinition
	{
		...
		private readonly Dictionary<string, GlobalStyle> _GlobalStyles;
		...
		{
            syntaxDefDict.TryGetValue(name, out SyntaxDefinition item);
		...
		{
			#region fields
			private readonly XmlHighlightingThemeDefinition _def;
			#endregion fields
			...
			public object VisitSyntaxDefinition(XshtdSyntaxDefinition syntax)
			{
				if (syntax.Name != null)
				{
					...
					if (_def.syntaxDefDict.ContainsKey(syntax.Name))
						throw Error(syntax, "Duplicate syntax definition name '" + syntax.Name + "'.");

					_def.syntaxDefDict.Add(syntax.Name, new SyntaxDefinition());
				}
			...
			public object VisitColor(XshtdSyntaxDefinition syntax, XshtdColor color)
			{
				if (color.Name != null)
				{
                    ...
                    if (_def.syntaxDefDict.TryGetValue(syntax.Name, out SyntaxDefinition synDef) == false)
			...
			public object VisitGlobalStyle(XshtdGlobalStyles globStyles, XshtdGlobalStyle style)
			{
				if (style.TypeName != null)
				{
                    ...
                    if (_def._GlobalStyles.TryGetValue(style.TypeName, out _) == true)
                        throw Error(style, "GlobalStyle definition '" + style.TypeName + "' has duplicates.");

                    GlobalStyle globDef = new(style.TypeName)
                    {
					...
					_def._GlobalStyles.Add(style.TypeName, new GlobalStyle(style.TypeName));
				}
		...
		{
			#region fields
			readonly XmlHighlightingThemeDefinition _def;
			#endregion fields
			...
			public object VisitSyntaxDefinition(XshtdSyntaxDefinition syntax)
			{
				SyntaxDefinition c;
				if (syntax.Name != null)
					c = _def.syntaxDefDict[syntax.Name];
				else
				{
			...
			public object VisitGlobalStyle(XshtdGlobalStyles globStyles, XshtdGlobalStyle style)
			{
                ...
                if (_def._GlobalStyles.TryGetValue(style.TypeName, out GlobalStyle globDef) == false)
